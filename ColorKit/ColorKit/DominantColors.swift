...
extension NativeImage {
    ...
    public func dominantColors(
        with quality: DominantColorQuality = .fair, algorithm: DominantColorAlgorithm = .iterative
    ) throws -> [NativeColor] {
        switch algorithm {
        case .iterative:
            let dominantColorFrequencies = try self.dominantColorFrequencies(with: quality)
            let dominantColors = dominantColorFrequencies.map { colorFrequency -> NativeColor in
    ...
    private func kMeansClustering(with quality: DominantColorQuality) throws -> [NativeColor] {
